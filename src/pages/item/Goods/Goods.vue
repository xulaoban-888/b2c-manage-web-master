    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon small v-if="props.item.saleable" @click="editSaleable(props.item.id)">下架</v-btn>
          <v-btn icon v-else @click="editSaleable(props.item.id)">上架</v-btn>
        </td>
      </template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      editSaleable(id){
        this.$http.put("/item/goods/saleable/"+id).then(()=>{
          this.getDataFromApi();
        })
      },
      getDataFromApi() {
